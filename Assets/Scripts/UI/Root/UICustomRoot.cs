...
namespace BlackAm
{
    public class UICustomRoot : SceneChangeEventUnityAsyncSingleton<UICustomRoot>
    {
        ...
        public void SetCanvasScaler(int p_Width, int p_Height)
        {
#if UNITY_EDITOR
            Debug.Log($"SetCanvasScaler : {p_Width}, {p_Height}");
#endif
            var canvas = _CanvasPresetGroup[RenderMode.ScreenSpaceCamera].Item1.GetComponentsInChildren<CanvasScaler>();
            foreach (var item in canvas)
            {
                item.referenceResolution = new Vector2(p_Width, p_Height);
            }
        }
