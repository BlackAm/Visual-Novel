...
namespace BlackAm
{
    public interface IDialogueModuleCluster : _IDisposable
    {
        IDialogue CurrentSelectedModule { get; }
        void SetModuleHandleIndex(int p_Index);
    }

    public class DialogueModuleCluster<ModuleLabelType, Module> : IDialogueModuleCluster where ModuleLabelType : struct where Module : class, IDialogue
    {
        ~DialogueModuleCluster()
        {
            Dispose();
        }
        ...
        private DialogueGameManager _MasterNode;

        /// <summary>
        /// 해당 유닛에 등록된 모듈
        /// </summary>
        public IDialogue CurrentSelectedModule { get; private set; }
        ...
        /// 현재 참조 중인 모듈 테이블
        /// </summary>
        private DialogueModuleDataTool.DialogueModuleType _DialogueModuleType;
        ...
        public DialogueModuleCluster(DialogueGameManager p_MasterNode, DialogueModuleDataTool.DialogueModuleType p_DialogueModuleType, List<int> p_LoadModuleIndexList)
        {
            _MasterNode = p_MasterNode;
            _DialogueModuleType = p_DialogueModuleType;
        ...
        private bool AddModule(int p_Index, bool p_UpdateDefaultIndex)
        {
            ...
            var (valid, tryModuleType) = DialogueModuleDataTool.GetDialogueTable<ModuleLabelType>(_DialogueModuleType).GetModuleType(p_Index);
            if (valid)
            {
                ...
                if (_ModuleTable.TryGetValue(tryModuleType, out var o_InnerTable))
                {
                    ...
                    {
                        var (spawnedModuleType, spawnedModule) = DialogueModuleDataTool.GetDialogueTable<ModuleLabelType>(_DialogueModuleType).SpawnModule(_MasterNode, p_Index);
        ...
        public Module SwitchModule(ModuleLabelType p_ModuleType)
        {
            ...
            {
                var (valid, fallbackIndex) = DialogueModuleDataTool.GetDialogueTable<ModuleLabelType>(_DialogueModuleType)?.GetDefaultFallbackIndex(p_ModuleType) ?? default;
        ...
        {
            var (valid, _ModuleType) = DialogueModuleDataTool.GetDialogueTable<ModuleLabelType>(_DialogueModuleType).GetModuleType(p_ModuleKey);
            if (valid)
            {
