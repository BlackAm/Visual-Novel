...
namespace UI2020
{
    public class SelectDialogueButton : AbstractUI
    {
        ...
        {
            dialogueGameManager.isDialogueEnd = true;
        ...
        {
            var selectDialogueCondition = SelectDialogueConditionPresetData.GetInstanceUnSafe[p_ConditionKey].SelectDialogueCondition;
            bool isInteractable = true;

            switch (selectDialogueCondition.Item1)
            {
                case DialogueGameManager.SelectDialogueCondition.None:
                    return isInteractable;
                case DialogueGameManager.SelectDialogueCondition.Liking:
                    isInteractable = CheckLiking(selectDialogueCondition.Item2);
                    break;
            }
        ...
        private bool CheckMoreLikingAnd(Dictionary<Character, int> p_LikingCollection, DialogueGameManager.LikingExtraType p_LikingExtraType = DialogueGameManager.LikingExtraType.AND)
        {
            ...
            {
                if (dialogueGameManager.CharacterLiking[character.Key] < character.Value)
                {
