...
namespace k514
{
    ...
    public struct ObjectDeployParams
    {
        ...
        /// 해당 이벤트의 시작 아핀 타입
        /// </summary>
        public Unit.UnitAttachPoint SpawnAttachPoint { get; private set; }
        ...
        public ObjectDeployParams(ObjectDeployEventPreset.EventDeployFlag p_EventDeployFlag = ObjectDeployEventPreset.EventDeployFlag.None, 
            int p_MaxHitCount = 0, int p_SameUnitHitCount = 0, float p_SameUnitHitInterval = 0f, float p_Range = 0f, 
            ObjectDeployEventPreset.AffineDeltaHandleType p_AffineDeltaHandleType = ObjectDeployEventPreset.AffineDeltaHandleType.None,
            Unit.UnitAttachPoint p_SpawnAttachPoint = Unit.UnitAttachPoint.MainTransform)
        {
            EventDeployFlag = p_EventDeployFlag;
            MaxHitCount = p_MaxHitCount;
            SameUnitHitPreset = (p_SameUnitHitCount, p_SameUnitHitInterval);
            Range = p_Range;
            SpawnAttachPoint = p_SpawnAttachPoint;
            AffineDeltaHandleType = p_AffineDeltaHandleType;

            if (p_SameUnitHitCount > 0)
            {
                if (p_SameUnitHitInterval < 1)
                {
                    SameUnitHitPreset = (p_SameUnitHitCount, 100000);
                }
    ...
    public struct ObjectDeployEventExtraPreset
    {
        ...
        /// <summary>
        /// MasterNode의 현재 타겟과는 별개로, 현재 배치 시점을 기준으로 타겟을 찾아야할 때 사용하는 필드
        /// 단 범위조건을 만족한다면 MasterNode의 포커스 유닛이 우선 선정된다.
        /// </summary>
        public (bool t_Valid, Unit t_Filtered) RangeTarget;
        ...
        {
            var result = true;
            _MasterNode = p_DeployEventRecord._MasterNode;
            ...
            // 배치 이벤트의 포커스 유닛을 선정한다.
            // 선정 순서는 다음과 같다.
            // 
            // 1. 범위 값이 0보다 큰 경우, 해당 범위를 기준으로 필터링을 수행
            // 2. 범위 값이 0보다 작고 _BlockFallBackCurrentFocus가 세트된 경우 포커스 유닛 없이 배치 이벤트를 수행함
            // 3. 범위 값이 0보다 작고 _BlockFallBackCurrentFocus가 세트되지 않은 경우 마스터노드의 포커스를 가져옴
            if (_MasterNode.IsValid())
            {
                /*RangeTarget = Range > 0f ? 
                    _MasterNode._MindObject
                        .FindEnemy(
                            StartPivot.Position, 
                            Range, 
                            ThinkableTool.AIUnitFindType.Random, 
                            Unit.UnitStateType.UnitFightableFilterMask
                        ) 
                    : (p_DeployEventRecord._BlockFallBackCurrentFocus ?
                        default
                        : _MasterNode.GetFocus()
                    );*/
            }

            if (RangeTarget.t_Valid)
            {
                TargetPivot = RangeTarget.t_Filtered.GetCenterPosition();
            }
            else
            {
                        ...
                        if (deployCollection.ContainsKey(deployableType))
                        {
                            switch (deployableType)
                            {
                                ...
                                case ObjectDeployTool.DeployableType.Projector:
                                    var projectorList = deployCollection[ObjectDeployTool.DeployableType.Projector];
                                    foreach (var i in projectorList)
                                    {
                                        ProjectorSpawnData.GetInstanceUnSafe.PreloadPrefab(i, ResourceLifeCycleType.Scene, 8);
                                    }
                                    break;
#endif
                                case ObjectDeployTool.DeployableType.Unit:
                                    var unitList = deployCollection[ObjectDeployTool.DeployableType.Unit];
                                    foreach (var i in unitList)
                                    {
                                        UnitSpawnManager.GetInstance.Preload(i, 8);
                                    }
                                    break;
...
/*#if !SERVER_DRIVE
                                case ObjectDeployTool.DeployableType.HitFilter:
                                    var hitFilterList = deployCollection[ObjectDeployTool.DeployableType.HitFilter];
                                    foreach (var i in hitFilterList)
                                    {
                                        var hitFilter = UnitHitPresetData.GetInstanceUnSafe.GetTableData(i);
                                        var hitMessage = hitFilter.GetHitMessage();
                                        if (hitMessage._HitStateFlag.HasAnyFlagExceptNone(HitMessage.HitMessageAttributeFlag.HasHitParameter))
                                        {
                                            var hitFXIndexTuple = hitMessage.GetHitParameter(UnitHitTool.HitParameterType.HitInteract);
                                            if (hitFXIndexTuple.Item1)
                                            {
                                                var hitFXCollection = UnitHitFxData.GetInstanceUnSafe.GetTableData(hitFXIndexTuple.Item2).HitFxMap;
                                                foreach (var FX_IndexTupleKV in hitFXCollection)
                                                {
                                                    var (vfxIndex, sfxIndex) = FX_IndexTupleKV.Value;
                                                    VfxSpawnManager.GetInstance.Preload(vfxIndex, 8);
                                                    SoundDataRoot.GetInstanceUnSafe.PreloadMediaClip(sfxIndex);
                                                }
                                            }
                                        }
                                    }
