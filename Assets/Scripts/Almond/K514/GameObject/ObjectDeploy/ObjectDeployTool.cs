...
namespace k514
{
    ...
    public static class ObjectDeployTool
    {
        ...
        /// </summary>
        public static (bool, Vector3) CorrectAffinePreset(int p_LayerMask, Vector3 p_Position, ObjectDeploySurfaceDeployType p_DeployFlagMask)
        {
            var (isValid, resultPosition) = (false, p_Position);
            var tryPosition = resultPosition;
            // 충돌 검증을 수행하는 경우
            if (p_DeployFlagMask.HasAnyFlagExceptNone(ObjectDeploySurfaceDeployType.HasCollisionCheck))
            {
                var collidedPos = default(Vector3);
                if (p_DeployFlagMask.HasAnyFlagExceptNone(ObjectDeploySurfaceDeployType.ForceSurface))
                {
                    (isValid, collidedPos) = PhysicsTool.GetHighestObjectPosition_RayCast(tryPosition, p_LayerMask, QueryTriggerInteraction.Collide);
                }
                else if (p_DeployFlagMask.HasAnyFlagExceptNone(ObjectDeploySurfaceDeployType.ForceSurfaceUsingParameterVector))
                {
                    (isValid, collidedPos) = PhysicsTool.GetHighestObjectPosition_CheckOverlap_RayCast(tryPosition, p_LayerMask, QueryTriggerInteraction.Collide);
                }
