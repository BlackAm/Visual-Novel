...
namespace k514
{
    public class SfxSpawnManager : SceneChangeEventSingleton<SfxSpawnManager>
    {
        ...
        private const int AudioObjectPreloadCount = 32;
...
#if !SERVER_DRIVE
        public int CalculateFootStepSound(FootStepLocation.TextureType pTextureType, Vocation p_Vocation)
        {
            var FootStepSound = 70000;
            switch (pTextureType)
            {
                case FootStepLocation.TextureType.Forest:
                    FootStepSound += FOREST_FOOTSTEP_SOUND;
                    break;
                case FootStepLocation.TextureType.Stone:
                    FootStepSound += STONE_FOOTSTEP_SOUND;
                    break;
                case FootStepLocation.TextureType.Sand:
                    FootStepSound += SAND_FOOTSTEP_SOUND;
                    break;
                case FootStepLocation.TextureType.Snow:
                    FootStepSound += SNOW_FOOTSTEP_SOUND;
                    break;
                case FootStepLocation.TextureType.DeepWater:
                    FootStepSound += DEEPWATER_FOOTSTEP_SOUND;
                    break;
                case FootStepLocation.TextureType.ShallowWater:
                    FootStepSound += SHALLOWWATER_FOOTSTEP_SOUND;
                    break;
            }
            ...
            switch (p_Vocation)
            {
                case Vocation.KNIGHT:
                    FootStepSound += KNIGHT_SOUND;
                    break;
                case Vocation.ARCHER:
                case Vocation.MAGICIAN:
                    FootStepSound += ARCHER_SOUND;
                    break;
            }
