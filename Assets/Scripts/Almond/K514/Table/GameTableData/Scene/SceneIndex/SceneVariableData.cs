...
namespace k514
{
    ...
    public class SceneVariableData : GameTable<SceneVariableData, int, SceneVariableData.TableRecord>
    {
        ...
        public class TableRecord : GameTableRecordBase
        {
            ...
            /// 현재 맵의 바닥 타입 리스트
            /// </summary>
            public FootStepLocation.TextureType TextureType { private set; get; }
            
            /// <summary>
            /// 현재 맵의 효과음 인덱스
            /// </summary>
            public int EffectSoundIndex { private set; get; }
            
#if UNITY_EDITOR && !SERVER_DRIVE
            public static TableRecord EditorModeCameraSettingManualTransmission(float CameraRootWrapperTiltDegree,
                float CameraRootWrapperSightDegree, float CameraDistance, Vector3 TraceOffset, CameraManager.TraceTargetPreset TraceTargetPreset)
            {
                var ManualData = new TableRecord();

                ManualData.CameraRootWrapperTiltDegree = CameraRootWrapperTiltDegree;
                ManualData.CameraRootWrapperSightDegree = CameraRootWrapperSightDegree;
                ManualData.CameraDistance = CameraDistance;
                ManualData.TraceOffset = TraceOffset;
