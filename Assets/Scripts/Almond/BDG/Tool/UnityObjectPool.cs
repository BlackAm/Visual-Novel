...
namespace BDG
{
    ...
    public abstract class UnityObjectPool<T> : MonoBehaviour where T : MonoBehaviour
    {
        ...
        private void CreateObject()
        {
            ...
            poolingObject.transform.SetParent(transform);
    ...
    public abstract class UnityPrefabObjectPool : MonoBehaviour
    {
        ...
        {
            var prefabObj = await LoadAssetManager.GetInstanceUnSafe.LoadAssetAsync<GameObject>(ResourceType.GameObjectPrefab, ResourceLifeCycleType.WholeGame, prefabName);
            var obj = Instantiate(prefabObj.Item2, transform, true);
            OnCreate(obj);
            OnPooled(obj);
            _pool.Push(obj);
        }
