...
public class ItemMeshChanger
{
    ...
    public async void ChangeMesh(MeshParts parts, int level)
    {
        ...
        var itemModel = await LoadAssetManager.GetInstanceUnSafe.LoadAssetAsync<GameObject>(ResourceType.GameObjectPrefab, ResourceLifeCycleType.Follwing, transform.name + level);
