...
namespace BlackAm
{
    public partial class DialogueGameManager
    {
        ...
        public int ShowSelectDialogue(int p_Key)
        {
            ...
            for (var i = selectDialogueInfoList.Count - 1; i >= 0; i--)
            {
                AddSelectDialogue(selectDialogueInfoList[i]);
            }
            
            MainGameUI.Instance.mainUI.ActiveMainUI(MainUI.UIList.TopCenter, MainGameUI.Instance.mainUI.IsUIActive(MainUI.UIList.BottomCenter));
            isDialogueEnd = true;
